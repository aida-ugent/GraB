...
class BgDistDegreeBlockEcoMemoryEfficient(BgDistBlockAbstract):
    # Constraint(s): total block sum and total node degree.

    def __init__(self, A, block_mask):
        super(BgDistDegreeBlockEcoMemoryEfficient, self).__init__(A, block_mask)
    ...
    def fit(self):
        ...
        P = csr_matrix(self._A.shape, dtype=np.float)
        # P[:] = np.nan
    ...
    def get_probability(self, row_ids, col_ids):
        return self.__P[row_ids, col_ids].toarray().ravel()
